# Third-Party Notices & Licenses

This project incorporates third-party open-source software packages and model services. 
Below are the notices and license texts required by their respective creators:

---

## 1. Express
- **License**: MIT License
- **Copyright**: Copyright (c) StrongLoop, Inc., and other expressjs.com contributors
- **URL**: https://github.com/expressjs/express

---

## 2. OpenAI Node SDK
- **License**: Apache License 2.0
- **Copyright**: Copyright (c) OpenAI
- **URL**: https://github.com/openai/openai-node

---

## 3. dotenv
- **License**: BSD-2-Clause License
- **Copyright**: Copyright (c) 2015, Mot Dotla
- **URL**: https://github.com/motdotla/dotenv

---

## 4. cors
- **License**: MIT License
- **Copyright**: Copyright (c) 2013 Troy Goode <troygoode@gmail.com>
- **URL**: https://github.com/expressjs/cors

---

## 5. Groq Cloud AI Services
- **License & Terms**: Groq Cloud Platform Terms of Service
- **Model**: openai/gpt-oss-120b
- **URL**: https://groq.com/
